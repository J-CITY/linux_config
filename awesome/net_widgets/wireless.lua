...
local function worker(args)
    ...
    net_icon:set_image(ICON_DIR.."wireless_na.svg")
	...
	awful.spawn.easy_async("awk 'NR==3 {printf \"%3.0f\" ,($3/70)*100}' /proc/net/wireless", function(stdout, stderr, reason, exit_code)
        ...
        if signal_level == nil then
            ...
            if signal_level < 25 then
                net_icon:set_image(ICON_DIR.."wireless_0.svg")
            elseif signal_level < 50 then
                net_icon:set_image(ICON_DIR.."wireless_1.svg")
            elseif signal_level < 75 then
                net_icon:set_image(ICON_DIR.."wireless_2.svg")
            else
                net_icon:set_image(ICON_DIR.."wireless_3.svg")
            end
        end
    end
